...
async function processVoiceCommand(mockText?: string) {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            ...
            if (!isListening) { vscode.window.showErrorMessage('Voice service timeout. Please try again.'); }
        } else {
            if (!isListening) { vscode.window.showErrorMessage(`Voice service error: ${error.message}`); }
        }
    }
}

async function handleMoveCursor(editor: vscode.TextEditor, targetLineOneBased: number) {
    ...
    if (!isListening) { vscode.window.showInformationMessage(`Cursor moved to line ${targetLineOneBased}`); }
}
...
async function handleRemoveLine(editor: vscode.TextEditor, line: number | undefined) {
    const document = editor.document;
